...
def chamando_coluna_linha(tam, tab):
    ...
    if inter > tam or linha > tam or inter < 0 or linha < 0:
        ...
        imprime_matriz(e, tam)
        return chamando_coluna_linha(tam, tab)
    if tab[linha - 1][inter] == BOLA_PRETA or tab[linha - 1][inter] == BOLA_BRANCA:
...
while True:
    ...
    movimentando_tab(linha1-1, coluna1, e, JOG1, BOLA_BRANCA, BOLA_PRETA)
    contar_liberdades(e)
    ...
    movimentando_tab(linha2-1, coluna2, e, JOG2, BOLA_BRANCA, BOLA_PRETA)
    contar_liberdades(e)
    capturando_innimigo(e)
    imprime_matriz(e, TAM)